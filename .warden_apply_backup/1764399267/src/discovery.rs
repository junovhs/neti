...
fn filter_config(files: Vec<PathBuf>, config: &Config) -> Result<Vec<PathBuf>> {
    let bin_re = Regex::new(BIN_EXT_PATTERN)?;
    let secret_re = Regex::new(SECRET_PATTERN)?;
    let code_re = if config.code_only {
...
fn should_keep_config(
    ...
    if secret_re.is_match(&s)
        || bin_re.is_match(&s)
        || config.exclude_patterns.iter().any(|p| p.is_match(&s))
    {
        return false;
    }

    if config.code_only {
        let is_code = code_re.is_some_and(|r| r.is_match(&s))
            || bare_re.is_some_and(|r| r.is_match(&s));
        if !is_code {
            return false;
        }
    }

    config.include_patterns.is_empty() || config.include_patterns.iter().any(|p| p.is_match(&s))
}